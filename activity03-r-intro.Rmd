...
**Response**: There's an error on using some punctuation as characters. Also, some characters were not separated with commas so I had to inset commas and remove some punctuation.
...
symbols = names(my_symbols)
```

Comment on what you noticed about the errors and how you used this information to correct the issues.

**Response**:The function data.frame was misspelt. (dataframe)
Symbol also had an upper case instead of all lowercase.and the function had been written wrongly. I changed the second line to symbols = names(my_symbols)
...
```{r}
Vector1 <- matrix(top_secret, ncol = 5)
evens <- matrix(c(seq(1,502,2)), ncol = 1)
Vector1 - evens
Vector1[18:24, 3]-100
Vector1[,4:5]*2
Vector1[, c(4,5)]*2
...
```{r}
stringr::str_c(my_symbols$Symbols[top_secret], collapse = "")
```
